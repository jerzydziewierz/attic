...
def make_on_message_callback(
    ...
    debug_mode = True
    if debug_mode:
        stdout(f'{len(state["streams"])=} {q_stream_idx=}')
    streamStateUpdate = dict(
        messageCount=0,
    ...
    state['streams'][q_stream_idx].update(streamStateUpdate)
    ...
    def curried_on_message(_1, _2, msg):
        nonlocal state
        state['streams'][q_stream_idx]['process_previous_start_time'] = \
            state['streams'][q_stream_idx]['process_start_time']
        state['streams'][q_stream_idx]['process_start_time'] = time.time()
        state['streams'][q_stream_idx]['previous_end_time'] = state['streams'][q_stream_idx][
            'process_end_time']
        ...
        def renew_database_link(
                state=state,
                q_stream_idx=None,
                q_stream_path=None) -> typing.Tuple[sqlite3.Cursor, sqlite3.Connection]:
            ...
            stdout(f'{q_stream_idx=} | topic={state['config']['streams'][q_stream_idx]['prefix']} |  creating new file {sqlite_db_path}')
            stdout(f'{sqlite_db_path=}')
        ...
        try:
            ...
            if state['streams'][q_stream_idx]['sqlite_cursor'] is None:
                # create the cursor and connection.
                # this is because the object must be created and used in the same thread.
                if not (state['streams'][q_stream_idx]['stop_request']):
                    _ = renew_database_link(state=state, q_stream_idx=q_stream_idx, q_stream_path=q_stream_path)
                else:
                    # except if the stop request has been issued, in which case, ignore the message and return.
                    sqlite_cursor = None
                    sqlite_connection = None
                    state['streams'][q_stream_idx]['stop_request_ack'] = True
                ...
                sqlite_cursor = state['streams'][q_stream_idx]['sqlite_cursor']
                sqlite_connection = state['streams'][q_stream_idx]['sqlite_connection']

                # if the stop request has been issued, make an effort to gracefully stop the database.
                if state['streams'][q_stream_idx]['stop_request']:
                    sqlite_connection.commit()
                    sqlite_cursor.close()
                    sqlite_connection.close()
                    state['streams'][q_stream_idx]['sqlite_cursor'] = None
                    state['streams'][q_stream_idx]['sqlite_connection'] = None
                    state['streams'][q_stream_idx]['stop_request_ack'] = True
                    return

                # Regular operation. The music doesn't stop! Make an effort to save the data.
                # but, first, check for rotation time
                next_rotation_time = state['streams'][q_stream_idx]['next_rotation_time']
            ...
            state['streams'][q_stream_idx]['lastRxTimestamp_unix'] = timestamp_unix
            state['streams'][q_stream_idx]['lastRxTimestamp_iso_string'] = timestamp_iso_string
            sqlite_cursor = state['streams'][q_stream_idx]['sqlite_cursor']
            if sqlite_cursor is not None:
                ...
                state['streams'][q_stream_idx]['messageCount'] += 1
                state['streams'][q_stream_idx]['process_end_time'] = time.time()
                # compute idle time
                q_idle_time = state['streams'][q_stream_idx]['process_start_time'] - \
                              state['streams'][q_stream_idx]['previous_end_time']
                # compute processing time
                q_processing_time = state['streams'][q_stream_idx]['process_end_time'] - \
                                    state['streams'][q_stream_idx]['process_start_time']
                # store the idle time and processing time
                # this is so that I can estimate the leftover node capacity.
                state['streams'][q_stream_idx]['totalIdleTime'] += q_idle_time
                state['streams'][q_stream_idx]['totalProcessingTime'] += q_processing_time
...
def signal_handler(sig, frame):
    ...
    # get state from the nearest enclosing scope
    state=None
    current_frame = frame
    while current_frame is not None:
        if 'state' in current_frame.f_locals:
            state = current_frame.f_locals['state']
            break
        current_frame = current_frame.f_back

    if state is None:
        print('shared_state not found in any frame. Aborting with fire.')
        return
    ...
    for stream_idx2 in range(len(state['streams'])):
        try:
            ...
            state['streams'][stream_idx2]['stop_request'] = True
            while not state['streams'][stream_idx2]['stop_request_ack']:
...
def run():
    stdout("starting hyattic, version 0.0.1")
    ...
    try:
        with open(config_path, 'r') as stream_idx:
            try:
                config = yaml.safe_load(stream_idx)
    ...
    for stream_idx in range(stream_count):
        stdout(f'stream {stream_idx}:')
        for key in config["streams"][stream_idx].keys():
            if key == 'password':
                stdout(f'{key:>30} -> {"*" * len(config["streams"][stream_idx][key])}')
            else:
                stdout(f'{key:>30} -> {config["streams"][stream_idx][key]}')
    ...
    state['streams'] = [dict() for _ in range(stream_count)]
    # !Obsolete: client is now property of stream
    # state['configs'] = []
    # state['clients'] = []

    for stream_idx in range(stream_count):
        stream_config = config['streams'][stream_idx] # config is read-only
        stream_state = state['streams'][stream_idx]
        #!obsolete shared_state['configs'].append(stream_config)
        ...
        client.on_connect = make_on_connect([stream_config['topic']])
        client.on_disconnect = on_disconnect

        if stream_config['user'] != "":
            client.username_pw_set(stream_config['user'], stream_config['password'])
        client.connect(stream_config['mqtt-broker-address'], stream_config['mqtt-broker-port'], 10)
    ...
    stats_last_message_count = 0
    stats_feedback_period = config['console-feedback-period-seconds']

    if config['log-performance']:
        performance_report_topic = config['log-performance-stream']['topic']
        performance_report_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2)
        if config['log-performance-stream']['user'] != "":
            performance_report_client.username_pw_set(config['log-performance-stream']['user'],
                                               config['log-performance-stream']['password'])
        performance_report_client.connect(config['log-performance-stream']['mqtt-broker-address'],
                                   config['log-performance-stream']['mqtt-broker-port'], 60)
        performance_report_client.loop_start()
    else:
        performance_report_client = None
        performance_report_topic = None

    # The Forever loop
    ##############################################################################################
    while True:
        # The main thread is a monitoring thread. The real work is done in the mqtt client threads.
        time.sleep(stats_feedback_period)

        current_time = time.time()
        elapsed_time = current_time - start_time
        totalMessageCount = state['totalMessageCount']
        messages_per_second_total = totalMessageCount / elapsed_time
        messages_per_second_recent = (totalMessageCount - stats_last_message_count) / stats_feedback_period
        stats_last_message_count = totalMessageCount
        elapsed_time_hours = elapsed_time / 3600
        performance_details = []
        # compute idle time to total time ratio, per stream
        # this is important to estimate the leftover node capacity.
        for stream_idx in range(len(state['streams'])):
            stream = state['streams'][stream_idx]
            idle_time = stream['totalIdleTime']
            processing_time = stream['totalProcessingTime']
            ...
            if total_time > 0:
                ...
                stream_connected = stream['client'].is_connected()
                if not stream_connected:
                    ...
                    try:
                        ...
                        troubled_client = stream['client']
                        stream_config = config['streams'][stream_idx]
                        time.sleep(0.1)
                        troubled_client.loop_stop()
                        time.sleep(0.1)
                        troubled_client.disconnect()
                        time.sleep(0.1)
                        # now, connect again with refreshed settings
                        if stream_config['user'] != "":
                            troubled_client.username_pw_set(username=stream_config['user'],
                                                     password=stream_config['password'])
                        time.sleep(0.1)
                        on_connect = make_on_connect([stream_config['topic']])
                        troubled_client.on_connect = on_connect  # this will also subscribe to the topic

                        troubled_client.connect(host=stream_config['mqtt-broker-address'],
                                         port=stream_config['mqtt-broker-port'],
                                         keepalive=10)
                        time.sleep(0.1)
                        troubled_client.loop_start()
                        time.sleep(0.1)
                    except Exception as ex:
                        import traceback
                ...
                performance_detail = dict(
                    ...
                    messages_this_channel=stream['messageCount'],
                    last_rx_timestamp_unix=stream['lastRxTimestamp_unix'],
                    last_rx_timestamp_iso=stream['lastRxTimestamp_iso_string'],
        ...
        if performance_report_client is not None:
            ...
            performance_report_client.publish(
                topic=performance_report_topic,
